...
class ToDoRenderer {
  ...
  renderTaskList(projectName) {
    const renderableTaskList = this.toDoProject.taskList.filter((tasks) => tasks.projects.includes(projectName));
    ...
    renderableTaskList.forEach((task) => {
      ...
      markImportantImage.src = task.importance === 'important' ? star : starOutline;
      ...
      if (task.dueDate !== '') {
        ...
        const dueDivPara = document.createElement('p');
  ...
  renderTaskDetials(todo) {
    ...
    const taskTitlePara = this.taskDetailsRenderDiv.querySelector(
      '#task-title-text-box',
    );
    const taskImageDiv = this.taskDetailsRenderDiv.querySelector('.task-image');
    ...
    if (todo.isPartOf('my-day')) {
      const myDayDiv = this.taskDetailsRenderDiv.querySelector('#my-day-div');
      const myDayDivPara = myDayDiv.querySelector('p');
    ...
    this.taskDetailsRenderDiv.querySelector('#due-date-para').textContent = todo.dueDate === '' ? 'Set due date' : `Due: ${todo.dueDate}`;

    this.taskDetailsRenderDiv.querySelector('#todo-notes').value = todo.notes;

    this.taskDetailsRenderDiv.querySelector(
      '.close-task-card>p',
    ).textContent = `Created: ${todo.creationDate}`;
  }

  renderSideNavBarCustomMenu() {
    const projectsList = this.customProject.customProjectList;
...
export default AllTaskRenderer;
